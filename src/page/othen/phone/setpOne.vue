<template>
  ...
  <div class="findPass_warp">
    ...
    <div class="findPassInfo_Pro">
      <epf-step :active="1" :data="data"  style="padding:0 130px;"></epf-step>
...
export default {
  ...
  mounted() {
   ...
   /* this.identifyCode =
      apiConfig.baseURL1 + "/epf-landweb/captcha.jpg?random=" + this.random;*/
    this.identifyCode =
  ...
  methods: {
    ...
    next(e) {
      if (e.phone && e.picCode && e.phoneValidate) {
        var that = this;
        //登录接口,//epf-auth已合并到epf-admin项目中
        // this.$axios.post('/epf-auth/jwt/token',qs.stringify({username:e.phone,phone:e.phone,captcha:e.picCode,random:this.random,smsCode:e.phoneValidate,loginType:'4'}),res=>{
        this.$axios.post(
          "/epf-cms/memberLogin",
          qs.stringify({
            username: e.phone,
          ...
          res => {
            ...
            /*this.$axios.post(
              "/epf-landweb/user/getUserInfoByToken",
              ...
              userInfoData => {
               ...
               // );
                ...
                //跳转到下一个页面
                this.$router.push("/bindPhone/setpSecond");
             /* }
            );*/
          }
        );
