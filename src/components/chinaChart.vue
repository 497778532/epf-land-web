<template>
  <div class="chart_message" :class="this.$route.path == '/'?'active':''">
    <div id="chinaSupplyChart" style="width:740px;height:575px;" ref="mapSupplyChart"></div>
    <div class="chart_table" v-if="this.$route.path == '/'">
      ...
      >
        ...
        <tr height="40px" v-for="(item,index) in useData" :key="index">
          ...
          <td>{{item.zrNum?item.zrNum:0}}</td>
          <td>{{item.czNum?item.czNum:0}}</td>
          <td>{{item.dyNum?item.dyNum:0}}</td>
        </tr>
      </table>
    </div>
...
export default {
  ...
  methods: {
    ...
    async loadMapSupply(mapCode, name) {
      debugger
      //请求数据
      ...
      this.$axios.get(
        ...
        `/epf-landweb/tsell/sellClassUseAnalys`,
        params,
        mapRes => {
          // let chartData = mapRes.list;旧的口数据
          //新接口数据mapRes.hotRes是左侧地图数据,mapRes.res是列表数据;
          let chartData = mapRes.hotRes;    ///新接口数据
          // console.log(chartData);

          var valueAllData = [];
          for (var key in chartData) {
            // valueAllData.push(chartData[key].counNumb);
            valueAllData.push(chartData[key].sellCount ? chartData[key].sellCount : 0); /////新接口数据
          }
          ...
          function getData(Data, AllData, shengM) {
            for (var key in Data) {
              AllData.push({
                name: shengM,
                value: Data[key].sellCount,
                // value: Data[key].counNumb,
                //////////////////////
                // gjData: Data[key].gjCounNumb,
                // xqData: Data[key].xqCounNumb,
                gjData: Data[key].sellCount,/////新接口
                ...
                // activeTabsName:this.activeTabsName
              });
            }
          }
          ...
          // console.log(this.valueAllDataMax)
      ...
      // this.valueAllDataMax=200
      // this.mapAllData = [
      //     {name:"北京",value:199,gjData:199},
      //     {name:"天津",value:42,gjData:42},
      //     {name:"河北",value:102,gjData:102},
      //     {name:"山西",value:81,gjData:81},
      //     {name:"内蒙古",value:47,gjData:47},
      //     {name:"辽宁",value:67,gjData:67},
      //     {name:"吉林",value:82,gjData:82},
      //     {name:"黑龙江",value:123,gjData:123},
      //     {name:"上海",value:24},
      //     {name:"江苏",value:92},
      //     {name:"浙江",value:114},
      //     {name:"安徽",value:109},
      ...
      // ];
      if (data) {
        this.chart = echarts.init(this.$refs.mapSupplyChart);
        // console.log(data)
          ...
          geo: {
            ...
            itemStyle: {
              ...
              emphasis: {
                ...
                // shadowBlur: 20,
                // borderWidth: 0,
                shadowColor:'#fff'//地图内部边框
                // shadowColor: "rgba(101,103,144, 0.15)"
              }
            }
          },
          series: [
        ...
        this.chart.on("click", function(params) {
          ...
          this.timeFn = setTimeout(function() {
            ...
            that.loadMapSupply(mapCode, name);
          }, 250);
        });
      } else {
        alert("无法加载该地图");
    ...
    returnLastStep: function() {
      ...
      this.chart.on("dblclick", function(params) {
        ...
        clearTimeout(this.timeFn);
        that.loadMapSupply("../../static/map/json/china.json", "china");
      });
    },
    useMsgSupply() {
      let params = this.params;
        ...
        res => {
          ...
          if(this.$route.path=="/"){
            if(res.res && res.res.length <= 1){
              ...
              res.res[0].circulation ==  "transfer" && res.res[0].counNumb ? res.res[0]['zrNum'] = res.res[0].counNumb  :  ''
              res.res[0].circulation ==  "rent"&&res.res[0].counNumb ?    res.res[0]['czNum'] = res.res[0].counNumb : ''
              res.res[0].circulation ==  "mortgage"&&res.res[0].counNumb ? res.res[0]['dyNum'] = res.res[0].counNumb : ''
              this.useData = res.res
              return;
            }else if(res.res && res.res.length >1){
              let typeFun = function(typeName){
                var typeData = res.res.filter((item,index,arr)=>{
                  if(item.circulation == typeName ){
                    return item
                  }
                })
                var numData = [];
                for (var key in typeData) {
                  ...
                  if(typeName == 'rent'){
                    obj['czNum'] = typeData[key].counNumb 
                  }else if(typeName == 'transfer'){
                    obj['zrNum'] = typeData[key].counNumb
                  }else if(typeName == 'mortgage'){
                    obj['dyNum'] = typeData[key].counNumb
                  }
                  numData.push(obj);
                }
                var result=[];
                ...
                for(var i=0;i<numData.length;i++){
                  ...
                  for(var j=0;j<result.length;j++){
                    if(numData[i].firstClassUse==result[j].firstClassUse){
                      flag=false;
                      if(typeName == 'rent'){
                        result[j].czNum=numData[i].czNum+result[j].czNum;
                      }else if(typeName == 'transfer'){
                        result[j].zrNum=numData[i].zrNum+result[j].zrNum;
                      }else if(typeName == 'mortgage'){
                        result[j].dyNum=numData[i].dyNum+result[j].dyNum;
                      }
                    }
                  }
                if(flag==true)
              ...
              let czData = typeFun('rent')
              let zrData = typeFun('transfer')
              let dyData = typeFun('mortgage')
              var newArr = czData.concat(zrData,dyData)
              newArr =  newArr.filter(function(item,index,arr){
                const i =  arr.find(_item=> item.firstClassUse === _item.firstClassUse);
                  if(i!==item){
                      i.czNum = i.czNum == undefined ? item.czNum:i.czNum;
                      i.zrNum = i.zrNum == undefined ? item.zrNum:i.zrNum;
                      i.dyNum = i.dyNum == undefined ? item.dyNum:i.dyNum;
                      return undefined;
                  }else{
                      i.czNum=item.czNum;
                      i.zrNum=item.zrNum;
          ...
          for (var key in res.res) {
            allNumberData.push(res.res[key].counNumb); //所有宗数
          }
          function sum(arr) {
          ...
          for (var key in res.res) {
            allData.push({
              firstClassUse: res.res[key].firstClassUse,
              firstClassUseName: res.res[key].firstClassUseName,
              number: res.res[key].counNumb,
              proportion: (
                (res.res[key].counNumb / allNumberSum) *
                100
              ).toFixed(2)
              // proportion: Math.round(
              //   (res.res[key].counNumb / allNumberSum) * 100
              // )
            });
            // console.log(allData)
          }
    ...
    changeSupplyMap() {
      if (sessionStorage.getItem("sdProvince")) {
        ...
        this.loadMapSupply("../../static/map/json/china.json", "china");
      }
    }
  },
  mounted() {
    this.$nextTick(function() {
      this.changeSupplyMap();
      this.useMsgSupply();
    });
    this.returnLastStep();
    this.getLastMonth()
